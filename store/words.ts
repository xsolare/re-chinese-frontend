...
export const useWordsStore = defineStore("words", {
  ...
  getters: {
    ...
    getNextWord (state): number {
      ...
      if (state.settings.random) { return words[Math.floor(Math.random() * (words.length - 1))].id; } else {
        const index = words.map(x => x.id).indexOf(state.currentWord.word.id);

        if (index === words.length - 1) { return words[0].id; } else { return words[index + 1].id; }
      }
    },
